(() => {
  ...
  if (window.__OUTING_PUSH_BRIDGE_V11842__) return;
  window.__OUTING_PUSH_BRIDGE_V11842__ = true;

  const getFrame = () => document.getElementById("outingFrame");

  function isKnownBridgeRequest(data) {
    const type = String(data?.type || "");
    return (
      type === "OUTING_PWA_HELLO_REQUEST" ||
      type === "OUTING_PUSH_STATUS" ||
      type === "OUTING_PUSH_SUBSCRIBE" ||
      type === "OUTING_BADGE"
    );
  }

  function isTrustedAppsScriptMessage(event, data) {
    ...
    /*
     * Some sandbox/nested-frame paths can surface an opaque "null" origin.
     * Accept ONLY our known bridge protocol in that case.
     *
     * OUTING_PUSH_SUBSCRIBE remains protected by the short-lived signed token
     * generated by Apps Script and validated by the Netlify subscribe endpoint.
     */
    if (origin === "null" && isKnownBridgeRequest(data)) {
      if (String(data?.type || "") === "OUTING_PUSH_SUBSCRIBE") {
        return !!(
          data &&
          data.token &&
          data.userKey &&
  ...
  window.addEventListener("message", async (event) => {
    ...
    if (!isKnownBridgeRequest(data)) return;
    if (!isTrustedAppsScriptMessage(event, data)) return;
